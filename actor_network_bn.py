...
class ActorNetwork:
    ...
    def create_network(self,state_dim,action_dim):
        ...
        layer0_bn = self.batch_norm_layer(h,training_phase=is_training,scope_bn='batch_norm_0',activation=tf.identity)
        ...
        params = detector_params+message_passing_params+[W1,b1,W2,b2,W3,b3];
        """
    ...
    def create_target_network(self,state_dim,action_dim,net):
        ...
        """
        layer0_bn = self.batch_norm_layer(h,training_phase=is_training,scope_bn='target_batch_norm_0',activation=tf.identity)
        layer1 = tf.matmul(layer0_bn,a_net[0]) + a_net[1]
        layer1_bn = self.batch_norm_layer(layer1,training_phase=is_training,scope_bn='target_batch_norm_1',activation=tf.nn.relu)
        layer2 = tf.matmul(layer1_bn,a_net[2]) + a_net[3]
        layer2_bn = self.batch_norm_layer(layer2,training_phase=is_training,scope_bn='target_batch_norm_2',activation=tf.nn.relu)

        action_output = tf.tanh(tf.matmul(layer2_bn,a_net[4]) + a_net[5])
        """
